...
def ventana_agregar():
    ...
    tk.Label(win, text="Cantidad:").grid(row=2, column=0)
    ...
    tk.Button(win, text="Guardar", command=agregar).grid(row=3, column=0, columnspan=3, pady=5)
...
def ventana_buscar():
    ...
    cols = ("Ítem", "Estado")
    tabla = ttk.Treeview(win, columns=cols, show="headings", height=8)
    for col in cols:
    ...
    def buscar():
        ...
        if nombre in pacientes:
            for i, item in enumerate(pacientes[nombre]):
                estado = "✔️" if item["estado"] else "❌"
                tabla.insert("", tk.END, values=(i, item["item"], estado))
        else:
            messagebox.showinfo("Resultado", "Paciente no encontrado.")

    def toggle_estado():
        seleccion = tabla.selection()
        if seleccion:
            idx = int(tabla.item(seleccion[0], "values")[0])
            nombre = entry_buscar.get().strip()
            if nombre in pacientes:
                pacientes[nombre][idx]["estado"] = not pacientes[nombre][idx]["estado"]
                guardar_excel()
                buscar()

    tk.Button(win, text="Buscar", command=buscar).pack(pady=5)
    tk.Button(win, text="Cambiar estado", command=toggle_estado).pack(pady=5)
